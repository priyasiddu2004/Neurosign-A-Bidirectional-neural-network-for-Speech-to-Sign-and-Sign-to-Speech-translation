...
import os
import numpy as np
# Removed unused imports - now using load_model instead of model_from_json
import operator
import time
import sys, os
...
class Application:
    def __init__(self):
        ...
        # Build models manually to avoid Keras 3.x compatibility issues
        import keras
        from keras.models import Sequential
        from keras.layers import Conv2D, MaxPooling2D, Flatten, Dense, Dropout, Input
        
        print("Building models manually to avoid compatibility issues...")
        
        # Build main model architecture (based on the JSON structure)
        ...
        # Build DRU model (3 classes: D, R, U)
        self.loaded_model_dru = Sequential([
            Conv2D(32, (3, 3), activation='relu', input_shape=(128, 128, 1)),
            MaxPooling2D((2, 2)),
            Conv2D(32, (3, 3), activation='relu'),
            MaxPooling2D((2, 2)),
            ...
            Dropout(0.4),
            Dense(64, activation='relu'),
            Dense(3, activation='softmax')  # 3 classes for DRU
        ])
        self.loaded_model_dru.load_weights(os.path.join(self.directory, "model-bw_dru.h5"))
        
        # Build TKDI model (4 classes: T, K, D, I)
        self.loaded_model_tkdi = Sequential([
            Conv2D(32, (3, 3), activation='relu', input_shape=(128, 128, 1)),
            MaxPooling2D((2, 2)),
            Conv2D(32, (3, 3), activation='relu'),
            MaxPooling2D((2, 2)),
            ...
            Dropout(0.4),
            Dense(64, activation='relu'),
            Dense(4, activation='softmax')  # 4 classes for TKDI
        ])
        self.loaded_model_tkdi.load_weights(os.path.join(self.directory, "model-bw_tkdi.h5"))
        
        # Build SMN model (3 classes: S, M, N)
        self.loaded_model_smn = Sequential([
            Conv2D(32, (3, 3), activation='relu', input_shape=(128, 128, 1)),
            MaxPooling2D((2, 2)),
            Conv2D(32, (3, 3), activation='relu'),
            MaxPooling2D((2, 2)),
            ...
            Dropout(0.4),
            Dense(64, activation='relu'),
            Dense(3, activation='softmax')  # 3 classes for SMN
        ])
        ...
        self.panel2.place(x = 460, y = 95, width = 310, height = 240)
        ...
        self.panel4.place(x = 220,y=560)
        self.T2 = tk.Label(self.root)
        self.T2.place(x = 10,y = 560)
        ...
        self.bt1=tk.Button(self.root, command=self.action1,height = 2,width = 15)
        self.bt1.place(x = 26,y=680)
        self.bt1.config(text="Suggestion 1", font=("Courier",10))
        
        self.bt2=tk.Button(self.root, command=self.action2,height = 2,width = 15)
        self.bt2.place(x = 180,y=680)
        self.bt2.config(text="Suggestion 2", font=("Courier",10))
        
        self.bt3=tk.Button(self.root, command=self.action3,height = 2,width = 15)
        self.bt3.place(x = 334,y=680)
        self.bt3.config(text="Suggestion 3", font=("Courier",10))
        
        self.bt4=tk.Button(self.root, command=self.action4,height = 2,width = 15)
        self.bt4.place(x = 488,y=680)
        self.bt4.config(text="Suggestion 4", font=("Courier",10))
        
        self.bt5=tk.Button(self.root, command=self.action5,height = 2,width = 15)
        self.bt5.place(x = 642,y=680)
        self.bt5.config(text="Suggestion 5", font=("Courier",10))
        
        # Add Clear button
        self.bt_clear=tk.Button(self.root, command=self.clear_all,height = 2,width = 15)
        self.bt_clear.place(x = 796,y=680)
        self.bt_clear.config(text="Clear All", font=("Courier",10), bg="red", fg="white")
        ...
        # Add timer for automatic word capture every 4 seconds
        self.word_timer = 0
        self.last_captured_letter = ""
        self.letter_count = 0
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            # Update display with smaller fonts to fit screen
            ...
            if(len(predicts) > 0):
                self.bt1.config(text=predicts[0][:10],font = ("Courier",10))  # Limit text length
            else:
                self.bt1.config(text="")
            if(len(predicts) > 1):
                self.bt2.config(text=predicts[1][:10],font = ("Courier",10))
            else:
                self.bt2.config(text="")
            if(len(predicts) > 2):
                self.bt3.config(text=predicts[2][:10],font = ("Courier",10))
            else:
                self.bt3.config(text="")
            if(len(predicts) > 3):
                self.bt4.config(text=predicts[3][:10],font = ("Courier",10))
            else:
                self.bt4.config(text="")
            if(len(predicts) > 4):
                self.bt5.config(text=predicts[4][:10],font = ("Courier",10))
            else:
                self.bt5.config(text="")
            
            # 4-second timer for automatic word capture
            self.word_timer += 1
            if self.word_timer >= 120:  # 120 frames = 4 seconds at 30fps
                self.word_timer = 0
                if self.current_symbol != 'blank' and self.current_symbol != 'Empty':
                    if self.current_symbol != self.last_captured_letter:
                        self.word += self.current_symbol
                        self.last_captured_letter = self.current_symbol
                        print(f"Auto-captured letter: {self.current_symbol}, Word: {self.word}")
    ...
    def predict(self,test_image):
        ...
        if(self.ct[self.current_symbol] > 30):  # Reduced from 60 to 30
            for i in ascii_uppercase:
                ...
                if tmp < 0:
                    tmp *= -1
                if tmp <= 10:  # Reduced from 20 to 10 for more sensitivity
                    self.ct['blank'] = 0
                    for i in ascii_uppercase:
                        self.ct[i] = 0
                    return
            ...
            if self.current_symbol == 'blank':
                ...
                self.blank_flag = 0
                self.word += self.current_symbol
                print(f"Added letter: {self.current_symbol}, Current word: {self.word}")  # Debug output
    def action1(self):
        predicts = list(self.hs.candidates(self.word))
        if len(predicts) > 0:
            self.word = ""
    ...
    def clear_all(self):
        """Clear all text and reset counters"""
        self.str = ""
        self.word = ""
        self.current_symbol = "Empty"
        self.ct['blank'] = 0

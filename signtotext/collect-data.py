...
# Additional words/phrases to capture
WORDS = [
    # Greetings
    "Hello", "Hi", "Good morning", "Good night", "Welcome", "Goodbye", "Bye",
    # Polite Words
    "Please", "Thank you", "Sorry", "Excuse me",
    # Basic Responses
    "Yes", "No", "OK", "Good", "Bad", "Help",
    # Emotions & Social
    "I love you", "Happy", "Sad", "Friend", "Family",
    # Common Questions
    "Who", "What", "Where", "When", "How",
    # Daily Use
    "Eat", "Drink", "Water", "Toilet", "Bathroom", "Stop", "Go", "Wait",
]
...
# Create directories for words/phrases
for word in WORDS:
    slug = slugify(word)
    if not os.path.exists("data/train/" + slug):
        os.makedirs("data/train/" + slug)
    if not os.path.exists("data/test/" + slug):
        os.makedirs("data/test/" + slug)
...
# Capture mode: 'letters' (default) or 'words'
capture_mode = 'letters'
...
while True:
    ...
    # Current word label and slug (for words mode)
    current_word = WORDS[current_word_index] if WORDS else ""
    current_slug = slugify(current_word) if current_word else ""
    ...
    # Header & instructions
    cv2.putText(frame, "MODE: "+mode+" | CAPTURE: "+capture_mode, (10, 30), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1)
    cv2.putText(frame, "[m] toggle letters/words", (10, 50), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1)
    if capture_mode == 'letters':
        ...
        cv2.putText(frame, "Press letter keys to save, [ESC] to exit", (10, 360), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1)
    else:
        # Words mode UI
        word_count = len(os.listdir(directory+current_slug)) if current_slug else 0
        cv2.putText(frame, "WORD: "+current_word+" ("+current_slug+")", (10, 80), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1)
        cv2.putText(frame, "COUNT: "+str(word_count), (10, 100), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1)
        cv2.putText(frame, "[ and ] select word | [space] save | [ESC] exit", (10, 120), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,255), 1)
...
#    _, roi = cv2.threshold(roi, 120, 255, cv2.THRESH_BINARY)
    ...
    if interrupt & 0xFF == ord('m'):
        capture_mode = 'words' if capture_mode == 'letters' else 'letters'
    # Navigate words list
    if capture_mode == 'words':
        if interrupt & 0xFF == ord(']'):
            current_word_index = (current_word_index + 1) % len(WORDS)
        if interrupt & 0xFF == ord('['):
            current_word_index = (current_word_index - 1) % len(WORDS)
        # Space to capture current word
        if interrupt & 0xFF == 32:  # spacebar
            word_count = len(os.listdir(directory+current_slug)) if current_slug else 0
            cv2.imwrite(directory+current_slug+'/'+str(word_count)+'.jpg', roi)
            continue

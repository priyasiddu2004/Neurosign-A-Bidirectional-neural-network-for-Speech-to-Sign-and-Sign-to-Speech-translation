...
export class MediapipeHolisticService {
  ...
  async load(): Promise<typeof holistic> {
    console.log('MediapipeHolisticService.load called');
    if (!this.importPromise) {
      console.log('Importing @mediapipe/holistic...');
      this.importPromise = import(/* webpackChunkName: "@mediapipe/holistic" */ '@mediapipe/holistic').then(
        module => {
          console.log('@mediapipe/holistic imported successfully');
          this.holistic = module.default;
          return this.holistic;
        }
      );
    }

    console.log('Waiting for holistic import...');
    const result = await this.importPromise;
    console.log('Holistic import completed');
    return result;
  }

...
export class PoseService {
  ...
  async load(): Promise<void> {
    console.log('PoseService.load called');
    if (!PoseService.loadPromise) {
      console.log('Creating new load promise');
    ...
    try {
      ...
      console.error('Pose model loading failed:', e);
      alert(e.message);
    }
  }

  private async _load(): Promise<void> {
    console.log('PoseService._load called');
    if (this.model) {
      console.log('Model already loaded');
      return;
    }

    console.log('Loading holistic service...');
    await this.holistic.load();
    console.log('Holistic service loaded');

    await this.ga.trace('pose', 'load', async () => {
      console.log('Creating Holistic model...');
      ...
      frame.width = 256;
      frame.height = 256;
      console.log('Sending empty frame to initialize...');
      await this.model.send({image: frame});
      frame.remove();
      console.log('Empty frame sent, model ready');
  ...
  async predict(video: HTMLVideoElement | HTMLImageElement): Promise<void> {
    console.log('PoseService.predict called with video:', video);
    await this.load();

    const frameType = this.isFirstFrame ? 'first-frame' : 'frame';
    console.log('Processing frame type:', frameType);
